...
!(function (root, factory) {
  ...
  function makeRule(node, opt, level) {
    ...
    if(reOneRule.test(props)) return indent + props[0] + ' ' + node.prop[props[0]] + ';\n'
    return [indent, getSelector(node, opt), '\n' ,
            indent , '{\n' ,
            props.map(function (v) {
              return indent + opt.indent + v + ': ' + node.prop[v] + ';'
            }).join('\n') , '\n' ,
            indent , '}' , '\n'
           ].join('')
  }
  ...
  function cssobj (obj, options, localNames) {
    ...
    // root[0].children._p.prop._color='bluesdfsdf'
