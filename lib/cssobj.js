...
!(function (root, factory) {
   ...
   */
  function convertSimpleData (d, opt, prop, parent) {
    parent = parent || {}
    if (is('Array', d)) {
      return d.map(function (v,i) {
        return convertSimpleData(v, opt, prop, {parent:parent, src:d, index:i})
      })
    }
    if (is('Object', d)) {
      var propArray = []
      ...
      for (var k in d) {
        ...
        if (isPrimitive(d[k])) {
          ...
          parent.children[k] = convertSimpleData(d[k], opt, prop, {parent:parent, src:d, key:k, value:d[k]})
        }
      }
  ...
  function cssobj (obj, options, localNames) {
    ...
    var p = convertSimpleData(obj, options)
    console.log(p)
