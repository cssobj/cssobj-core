...
!(function (root, factory) {
  ...
  var is = util._is
  var own = util._own
  var isValueType = function(v){return util._isPrim(v) || is('Function',v)}
  ...
  function parseObj (d, opt, parent) {
    ...
    if (is('Object', d)) {
      ...
      for (var k in d) {
        if (!own(d, k)) continue
        if (isValueType(d[k]) || is('Array', d[k]) && isValueType(d[k][0])) {
          ![].concat(d[k]).forEach(function(v){
            if(k=='$id') opt.vars[v] = parent
  ...
  function makeRule(node, opt, level) {
    ...
    var getVal = function(indent, key, sep, end){
      ...
      return propArr.map(function(t){
        return indent + key + sep + (is('Function', t)
                                     ? t(node, getSelector(node, opt), opt)
                                     : t) + end
      }).join('')
    }
    ...
    props.forEach(function(v){
      if(reOneRule.test(v)) str += getVal(indent, v, ' ', ';\n')
    })
    ...
    return !selector
      ...
      : str + [indent, selector , '\n' ,
         ...
         props.map(function (v) {
           return getVal(indent + opt.indent, v, ': ', ';\n')
         }).join('') ,
         indent , '}' , '\n'
        ].join('')
  }
  ...
  function makeCSS(node, opt, recursive) {
    ...
    var newGroup = function(node, cur){
      ...
      var selector = rule + path.reduce(function(pre, cur){
        return cur.replace(reGroupRule,'').split(',').map(function(v) {
          return !pre ? v : pre.replace(reGroupRule,'').split(',').map(function(p){
            return p + ' and ' + v
          }).join(',')
        }).join(',\n')
      }, '')
    ...
    var walk = function (node, groupLevel) {
      ...
      var isInKeyFrame = !!getParent(node.parent, function(v){return reKeyFrame.test(v.key)})
      if(isGroupRule) {
        groupLevel++
        cur = []
        newGroup(node, cur)
      ...
      if(isKeyFrameNode) {
        cur.push(_repeat(opt.indent, indentLevel) + node.key+'{\n')
      }
      ...
      if(isKeyFrameNode){
        cur.push(_repeat(opt.indent, indentLevel) + '}\n')
      }
      if(isGroupRule) {
        cur.push('}\n')
        groupLevel--
