...
!(function (root, factory) {
  ...
  function makeCSS(node, opt, recursive) {
    ...
    var getGroupArr = function(node, newRule){
      var p=node, path=[]
      while(p) {
        if( reGroupRule.test(p.key) ) path.unshift(p.key)
        p = p.parent
      }
      if(!newRule) return path.length
    ...
    var getLevel = function(node){return !!getGroupArr(node) + (node.parent && reKeyFrame.test(node.parent.key))|0}
    ...
    var walk = function (node, groupLevel) {
      ...
      if(isGroupRule) {
        groupLevel++
        cur = getGroupArr(node, true)
      }
      if(isKeyFrame) {
        cur.push(_repeat(opt.indent, getLevel(node)) + node.key+'{\n')
      }
      ...
      if(isKeyFrame){
        cur.push(_repeat(opt.indent, getLevel(node)) + '}\n')
      }
      if(isGroupRule) {
        cur.push('}\n')
        groupLevel--
