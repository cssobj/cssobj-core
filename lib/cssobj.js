...
!(function (root, factory) {
  ...
  function getSelector (node, opt) {
    var replacer = function (match, global, dot, name) {
      ...
      if (!opt.local) {
        ...
        opt.NS[name] = opt.prefix + name
      }

      return dot + opt.NS[name]
    }
  ...
  function makeRule(node, opt, level) {
    ...
    if(reOneRule.test(node.prop)) return indent + node.prop[0].join(' ') + ';\n'
    return [indent, getSelector(node, opt), '\n' ,
            indent , '{\n' ,
            node.prop.map(function (v) {
              return indent + opt.indent + v.join(': ') + ';'
            }).join('\n') , '\n' ,
            indent , '}' , '\n'
           ].join('')
  }
  ...
  function cssobj (obj, options, localNames) {
    ...
    var walk = function (node) {
      ...
      if(isKeyFrame) {
        curStr().push(_repeat(options.indent, getLevel(node)) + node.key+'{\n')
      }

      if (node.prop.length)
        curStr().push(
          makeRule(node, options, getLevel(node))
        )
      for(var k in node.children)
        walk(node.children[k])

      if(isKeyFrame){
        curStr().push(_repeat(options.indent, getLevel(node)) + '}\n')
      }
      if(isGroupRule) {
        curStr().push('}\n')
        groupArr.pop()
