!(function (root, factory) {
  ...
  var propStart = function (opt, level) {return '\n' + _repeat(opt.indent, level) + '{\n'}
  var propEnd = function (opt, level) {return '\n' + _repeat(opt.indent, level) + '}'}
  ...
  function propFormatter (propArray, opt, level) {
    return propArray.map(function (v) {return _repeat(opt.indent, level) + v + ';'}).join('\n')
  }
  ...
  function makeRule(sel, body, opt, level) {
  ...
  function getCSS (store, opt, level) {
    ...
    for(k in store){
      ...
      if(type(store[k])==='Object') {
        s2.push( makeRule(k, getCSS(store[k], opt, level+1), opt, level) )
      } else {
        s1.push( store[k].map(function(p) {
          return makeRule(k, propFormatter(p, opt, level+1), opt, level)
        }).join('\n') )
      }
    }
