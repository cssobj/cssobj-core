...
var suffix = '_'+Math.floor(Math.random() * Math.pow(2,32)).toString(36) + '_' + Math.floor(Math.random() * Math.pow(2,32)).toString(36)
var count = 0
...
function replacer(match, global, dot, name) {
  ...
  if (!localNames[name]) localNames[name] = name + suffix
