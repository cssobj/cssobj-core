!(function (root, factory) {
  ...
  function convertSimpleData (d, opt, prop, path) {
    ...
    if (type(d) === 'Object') {
      ...
      for (var k in d) {
        if(isCondition(k)){
          // var s={store:{}}
          // console.log(d[k],k, convertSimpleData(d[k], s, null, [{name:k, value:d[k]}]) )
          // console.log(s, path)
        }
      ...
      if (path.length) objKV(opt.store, getSelector(path, opt), propArray)
      return node
    }
    return []
  }
  ...
  function getSelector (path, opt) {
    var replacer = function (match, global, dot, name) {
      ...
      if (!opt.localNames[name]) opt.localNames[name] = opt.local
  ...
  function getCSS (opt) {
    return Object.keys(opt.store).map(
      function (k) {
        return opt.store[k].map(function(p,i) {
          return k +
            propStart() +
            propFormatter(p, opt) +
            propEnd()
  ...
  function cssobj (obj, options) {
    options = options || {}
    var defaultOption = {local:true, indent:'\t', store:{}, localNames:{}}
    ...
    var result = {css: getCSS(options), map: options.localNames, store:options.store, options:options}
    if (options.post) options.post.forEach(function (f) { f(result) })
    delete options.store
    delete options.localNames
    return result
  }
