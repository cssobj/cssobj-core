...
!(function (root, factory) {
  ...
  var reAtRule = /^@(?:media|document|supports)/
  ...
  function getSelector (node, opt) {
    ...
    for (var i = 0, len = path.length; i < len; i++) {
      ...
      if(reAtRule.test(item.key)) continue
      if (!item.selector) {
  ...
  function cssobj (obj, options, localNames) {
    ...
    var walk = function (node) {
      ...
      var atRule = reAtRule.test(node.key)
      if(atRule) {
        str += _repeat(options.indent, level) + node.key + '{\n'
        level++
      }

      if (node.prop.length) str += makeRule(node, options, level)
      ...
      if(atRule) {
        level--
        str += _repeat(options.indent, level) + '}\n'
      }
    }
    walk(p)
